...
| 刷字库 | 将 6766 个中文字符字库写入 SPI Flash，支持信道中文命名 |
| 备份校准 | 导出设备校准数据备份 |
| 恢复校准 | 从备份文件恢复校准数据 |
...
| 字符数量 | 6766 个（GB2312完整字符集） |
| 字库大小 | 218,613 字节 (约 213.5 KB) |
| SPI Flash 占用 | 10.4% (总容量 2MB) |
...
- 字库格式：12×12 点阵位图 + Unicode 索引表（6字节条目） + 拼音查找表
